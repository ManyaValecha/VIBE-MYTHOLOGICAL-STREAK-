...
const REAL_ANNOUNCEMENTS = [
  {
    ...
    date: "2026-07-10",
    author: "Acharya Manya Valecha (Core Architect)",
    tag: "Summership",
    isPinned: true
  },
  {
...
// API: Get raw lesson content from vicharanashala/vibe
app.get("/api/lesson-content", async (req, res) => {
  ...
  // Split path segments, encode each segment individually, and join them with slashes
  // E.g. "Typescript" and "1. Introduction to TypeScript/1.1 Introduction to Typescript.md"
  const fullRelativePath = `${category as string}/${lesson as string}`;
  const encodedPath = fullRelativePath.split('/').map(seg => encodeURIComponent(seg)).join('/');
  
  const fileUrl = `https://raw.githubusercontent.com/vicharanashala/vibe/main/docs/newdocs/${encodedPath}`;
  ...
  try {
    ...
    console.error("Failed to fetch markdown from GitHub:", error.message);
...
// API: Get Vibe GitHub Contributors
...
// Serve Vite dev server or static distribution
